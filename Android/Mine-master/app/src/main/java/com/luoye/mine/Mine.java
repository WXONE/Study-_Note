...
public class Mine {
    ...
    //游戏中的块
    public   class Tile{
        short value;//雷块：-1 空白：0 数字：> 0
        boolean flag;//存储该雷是否被标记
        boolean open;//存储该块是否被点开过
        public Tile()
        {
            this.value=0;
            this.flag=false;
    ...
    public static class Point{
        ...
        //重写hashCOde和equals方法，比较位置与位置是否相同
    ...
    public void create(Point exception)
    {
        ...
        //随机产生雷
        //先把地图中除p位置外所有的位置加入到链表中，然后生成0到链表大小-1之间的随机数，根据生成的随机数在链表中取元素，
        // 取完元素就把该位置从链表中移除，并把Tile数组中该位置的Tile的value设为-1
        ...
        //给地图添加数字
        //遍历Tile数组，遇到雷就将他身边的八个的位置的value值加1，如果八个位置中有雷，或者该位置不存在，不执行任何操作
        for (int i = 0; i< mapRow; i++)//y
        {
            for (int j = 0; j< mapCol; j++)//x
            {
